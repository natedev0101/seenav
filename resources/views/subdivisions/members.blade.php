<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">
            {{ __('Osztály részletei:') }} <span style="color: {{ $subdivision->color }}">{{ $subdivision->name }}</span>
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-gray-800 overflow-hidden shadow-md rounded-lg">
                <div class="p-6 text-white space-y-6">
                    ...
                    {{-- Kereső panel --}}
                    <div class="mb-6">
                        <form method="GET" action="{{ route('subdivisions.users', $subdivision) }}" class="flex gap-4">
                            <div class="flex-1">
                                <input type="text" 
                                       name="search" 
                                       value="{{ $search }}" 
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="text-center py-4 text-gray-400">
                                            ...
                                            @else
                                                {{ __('Nincs felhasználó ebben az alosztályban.') }}
                                            @endif
                                        </td>
                                    </tr>
                                @endforelse
